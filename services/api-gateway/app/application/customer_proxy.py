...
class CustomerServiceProxy:
    ...
    async def forward(self, request: Request, upstream_path: str) -> Response:
        ...
        try:
            ...
            self._metrics.observe_upstream(
                "customer-service", "timeout", None, perf_counter() - started_at
            )
            logger.warning(
            ...
            self._metrics.observe_upstream(
                "customer-service", "unavailable", None, perf_counter() - started_at
            )
            logger.warning(
            ...
            self._metrics.observe_upstream(
                "customer-service", "transport_error", None, perf_counter() - started_at
            )
            logger.warning(
        ...
        self._metrics.observe_upstream(
            "customer-service", result, upstream.status_code, duration_seconds
        )
        logger.info(
    ...
    async def is_ready(self, request_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "customer_service_readiness_timeout",
                extra={
