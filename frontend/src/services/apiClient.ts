...
function messageForStatus(status: number): string {
  ...
  if (status === 404) return "The requested customer record was not found.";
  if (status === 409) return "The request conflicts with the current customer record.";
  if (status === 503 || status === 504)
    return "The customer capability is temporarily unavailable.";
