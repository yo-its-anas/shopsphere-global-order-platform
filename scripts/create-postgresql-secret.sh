...
main() {
    ...
    if kubectl --context "$KUBE_CONTEXT" -n "$NAMESPACE" get secret "$SECRET_NAME" >/dev/null 2>&1; then
        ...
        if [[ "$catalogue_key_state" == "present" && "$order_key_state" == "present" ]]; then
            info "Secret '${SECRET_NAME}' already contains all service database credentials; it was preserved without changes."
            return
        fi
        ...
        if [[ "$order_key_state" != "present" ]]; then
            if [[ "$mode" == "--generate" ]]; then
                order_password="$(generate_password)"
            else
                [[ -t 0 ]] || fail "Interactive password entry requires a terminal. Use --generate only when automatic generation is explicitly intended."
                read_password "order-service database password" order_password
            fi
        fi

        if [[ "$catalogue_key_state" != "present" && "$order_key_state" != "present" ]]; then
            printf '{"data":{"catalogue-password":"%s","order-password":"%s"}}' \
                "$(encode "$catalogue_password")" "$(encode "$order_password")"
        elif [[ "$catalogue_key_state" != "present" ]]; then
            printf '{"data":{"catalogue-password":"%s"}}' "$(encode "$catalogue_password")"
        else
            printf '{"data":{"order-password":"%s"}}' "$(encode "$order_password")"
        fi | kubectl --context "$KUBE_CONTEXT" -n "$NAMESPACE" patch secret "$SECRET_NAME" \
            --type merge --patch-file=/dev/stdin >/dev/null
        unset catalogue_password order_password
        info "Missing service database credentials were added to Secret '${SECRET_NAME}'. Existing keys were preserved and no value was displayed."
        return
    fi
    ...
    printf '{"apiVersion":"v1","kind":"Secret","metadata":{"name":"%s","namespace":"%s"},"type":"Opaque","data":{"postgres-password":"%s","customer-password":"%s","keycloak-password":"%s","catalogue-password":"%s","order-password":"%s"}}' \
        "$SECRET_NAME" \
        "$NAMESPACE" \
        ...
        "$(encode "$order_password")" | \
        kubectl --context "$KUBE_CONTEXT" create -f - >/dev/null

    unset postgres_password customer_password keycloak_password catalogue_password order_password
