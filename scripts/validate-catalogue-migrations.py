#!/usr/bin/env python3
"""Validate the catalogue Alembic revision graph without connecting to a database."""
...
def main() -> int:
    ...
    service_directory = repository_root / "services" / "catalogue-service"
    ...
    if len(heads) != 1:
        raise RuntimeError("Catalogue migrations must have exactly one head revision.")
    if len(bases) != 1:
        raise RuntimeError("Catalogue migrations must have exactly one base revision.")
    if len({revision.revision for revision in revisions}) != len(revisions):
        raise RuntimeError(
            "Catalogue migrations contain duplicate revision identifiers."
        )
    ...
    print(
        "Catalogue migration integrity passed: "
