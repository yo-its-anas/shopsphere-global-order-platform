...
readonly TARGET_SECRET="shopsphere-customer-service-database"
...
main() {
    ...
    password_base64="$(kubectl --context "$KUBE_CONTEXT" -n "$SOURCE_NAMESPACE" \
        get secret "$SOURCE_SECRET" -o jsonpath='{.data.customer-password}')"
    [[ -n "$password_base64" ]] || \
        fail "The existing PostgreSQL Secret does not contain key 'customer-password'."
    ...
    database_url="postgresql+psycopg://customer_app:${encoded_password}@postgresql.shopsphere-data.svc.cluster.local:5432/customer_db"
