...
readonly OVERLAY="${REPOSITORY_ROOT}/platform/kubernetes/overlays/poc/grafana"
...
main() {
  ...
  grep -q 'image: grafana/grafana:11.2.0' <<<"${rendered}" ||
    fail 'The reviewed Grafana image version is not pinned.'
  grep -q 'type: ClusterIP' <<<"${rendered}" ||
    fail 'Grafana must use a ClusterIP Service.'
  ...
  grep -q 'runAsNonRoot: true' <<<"${rendered}" ||
    fail 'Non-root execution is required for Grafana.'
  grep -q 'readOnlyRootFilesystem: true' <<<"${rendered}" ||
    fail 'A read-only root filesystem is required.'

  printf '[OK] Grafana manifests passed static safety and configuration validation.\n'
}
