...
readonly TARGET_SECRET="shopsphere-order-service-database"
...
main() {
    ...
    password_base64="$(kubectl --context "$KUBE_CONTEXT" -n "$SOURCE_NAMESPACE" \
        get secret "$SOURCE_SECRET" -o jsonpath='{.data.order-password}')" || \
        fail "The PostgreSQL credential Secret was not found."
    [[ -n "$password_base64" ]] || \
        fail "The existing PostgreSQL Secret does not contain key 'order-password'."
    ...
    database_url="postgresql+psycopg://order_app:${encoded_password}@postgresql.shopsphere-data.svc.cluster.local:5432/order_db"
