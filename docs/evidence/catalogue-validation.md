...
API/domain/cache tests use in-memory repository and cache contract implementations for deterministic behavior. Redis adapter tests cover TTL forwarding, malformed JSON, and Redis exceptions. The earlier evidence validated SQLAlchemy/Alembic against disposable PostgreSQL through migration head `003_domain_event_outbox`. Reservation source and migration `004_inventory_reservations` now pass isolated tests and offline PostgreSQL SQL compilation, but have not been applied to disposable or live PostgreSQL. Gateway tests use an injected HTTP transport; reservation routes are intentionally not registered there. Single-product reserve/retrieve/release/consume behavior is Unit Validated only; service identity, deployment, live Kafka topics, and order-service integration remain Pending / Not Verified. API Gateway, Redis, Kafka, PostgreSQL, and catalogue-service each run on the same kind node and are not highly available.
