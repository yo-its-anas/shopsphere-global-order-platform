...
API/domain/cache tests use in-memory repository and cache contract implementations for deterministic behavior. Redis adapter tests cover TTL forwarding, malformed JSON, and Redis exceptions. SQLAlchemy metadata and Alembic were also validated against disposable PostgreSQL, including migration round trip, drift, append-only trigger, and balance constraints. Gateway tests use an injected HTTP transport; the deployed gateway additionally forwarded an unauthenticated catalogue request to authoritative backend authentication and preserved the resulting `401`. No authenticated live product mutation/read journey through the gateway was executed. Order reservation/release/fulfilment behavior is not implemented. API Gateway, Redis, Kafka, PostgreSQL, and catalogue-service each run on the same kind node and are not highly available.
