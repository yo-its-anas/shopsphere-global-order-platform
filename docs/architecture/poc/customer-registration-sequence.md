...
sequenceDiagram
    autonumber
    actor Customer
    participant React as React SPA<br/>(integration planned)
    participant Keycloak as Keycloak<br/>identity authority
    participant Gateway as API Gateway<br/>(routing planned)
    ...
    Gateway->>Service: Forward authenticated request<br/>(gateway enforcement planned)
...
React OIDC integration and API-gateway routing remain planned, so the complete browser-to-service sequence is not claimed as an executed end-to-end journey. Tests exercise the service portion with signed access tokens and isolated PostgreSQL.
